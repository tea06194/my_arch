...
function M.setup(opts)
	-- Check Hyprland
	if vim.fn.system('echo $XDG_CURRENT_DESKTOP'):find('Hyprland') == nil and
		vim.fn.system('echo $HYPRLAND_INSTANCE_SIGNATURE') == '' then
		error("(hyprland-keyboard-switch.lua) Error: This plugin works only in Hyprland environment")
		return
	end
	...
	opts = opts or {}
	if opts.device then
		M.device = opts.device
	end
	if opts.us_layout_index then
		M.us_layout_index = opts.us_layout_index
	end

	get_current_layout_index_async(function(init_idx)
		if init_idx and init_idx ~= M.us_layout_index then
